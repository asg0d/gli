...
from .models import Employee, Billboard, BillboardImage

@admin.register(Employee)
class EmployeeAdmin(admin.ModelAdmin):
    list_display = ['full_name', 'email', 'phone', 'position', 'billboards_count', 'is_active']
    ...
    def billboards_count(self, obj):
        ...
        if count > 0:
            url = reverse('admin:billboards_billboard_changelist') + f'?employee__id__exact={obj.id}'
...
class BillboardAdmin(admin.ModelAdmin):
    list_display = [
        'id', 'title', 'employee', 'size_display', 'status_badge', 
        'period_display', 'days_left', 'created_at'
    ]
    list_filter = ['status', 'employee', 'created_at', 'start_date', 'end_date']
    search_fields = ['title', 'address', 'employee__first_name', 'employee__last_name']
    readonly_fields = ['created_at', 'updated_at', 'days_until_expiry']
    inlines = [BillboardImageInline]
    
    fieldsets = (
        ('Основная информация', {
            'fields': ('title', 'description', 'employee', 'status')
        }),
...
class BillboardImageAdmin(admin.ModelAdmin):
    list_display = ['billboard', 'image_preview', 'alt_text', 'is_primary', 'order', 'uploaded_at']
    list_filter = ['is_primary', 'uploaded_at', 'billboard__status']
